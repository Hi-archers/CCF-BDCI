...
#             print(tokenizer.decode(source_tokens[0], skip_special_tokens=True).replace(" ",""))
#             print(tokenizer.decode(target_tokens[0], skip_special_tokens=True).replace(" ",""))
...
#             exit(0)
            ...
            if len(tmp) == 0:
                ...
                print(f"\n生成结果\n{tokenizer.decode(tmp, skip_special_tokens=True)}\n实际结果\n{tokenizer.decode(target_tokens[0], skip_special_tokens=False)}")
                logger.info(tokenizer.decode(tmp, skip_special_tokens=False))

        logger.info(f"epoch = {epoch} train loss = {sum(loss_train)/len(loss_train)}")


        model.save_pretrained(os.path.join("./ckpt/",str(epoch) + '_ckpt'))
