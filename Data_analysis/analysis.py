...
def train_length():
    ...
    plot_bar(sorted_text_len, title='Train text length', xlabel='', ylabel='Text Length')
    ...
    plot_bar(sorted_sent_len, title='Train sentence length', xlabel='', ylabel='Sent Length')
...
def valid_length():
    ...
    plot_bar(sorted_text_len, title='Valid text length', xlabel='', ylabel='Text Length')
    ...
    plot_bar(sorted_sent_len, title='Valid sentence length', xlabel='', ylabel='Sent Length')
...
def entity_pos():
                ...
                if sent.find(h_name) != -1 and sent.find(t_name) != -1:
                    ...
                    if h_num >= 2 or t_num >= 2:
                        ...
                        if h_num >= 2:
                            ...
                            entity_dic[now_id]['多次出现的头实体'].append({"hname": h_name, "times": h_num, "sent": sent})
                        if t_num >= 2:
    ...
    plot_bar_count(hname_dic, title='训练集头实体长度(去重)', xlabel='头实体长度', ylabel='个数', xlim=False, low=0, step=1)
    plot_bar_count(tname_dic, title='训练集尾实体长度(去重)', xlabel='尾实体长度', ylabel='个数', xlim=False, low=0, step=1)
    ...
    plot_bar_count(dict(Counter(hnum_cnt)), title='头尾实体在同一个句子内情况下，头实体在一个句子中出现次数', xlabel='在一个句子中出现次数', ylabel='Count')
    plot_bar_count(dict(Counter(tnum_cnt)), title='头尾实体在同一个句子内情况下，尾实体在一个句子中出现次数', xlabel='在一个句子中出现次数', ylabel='Count')
