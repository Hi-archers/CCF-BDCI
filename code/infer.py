...
def infer(args, logger):
    ...
    for i,batch in enumerate(tqdm(infer_dataloader)):
        ...
        for i in range(len(outputs[0])):
            output_decode = []
            for j in range(10):
                output = ''.join(tokenizer.decode(outputs[j][i], skip_special_tokens = True)).replace(' ','')
                output_decode.append(output) 
            outputs_decode.append([batch['meta_data'][i]['id'],batch['meta_data'][i]['condition'],output_decode])
...
def Get_Entity(pred):
    flag = False
    for i in pred:
        head_entity = re.findall(r'头:(.+?)\$\$关系:',i)
        tail_entity = re.findall(r'\$\$尾:(.+?)',i)
        relation = re.findall(r'\$\$关系:(.+?)\$\$尾:',i)
        if len(head_entity) != 0 and len(tail_entity) != 0 and len(relation) != 0:
